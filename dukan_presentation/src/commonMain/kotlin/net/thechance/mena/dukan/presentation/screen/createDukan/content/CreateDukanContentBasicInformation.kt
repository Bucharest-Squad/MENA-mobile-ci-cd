...
private fun DukanNameSection(
    ...
    TextField(
        value = state.name,
        onValueChanged = { name -> listener.onNameChanged(name) },
        ...
        state.availableCategories.forEach { category ->
            item {
                Chip(
                    ...
                    isEnabled = state.selectedCategories.contains(category) || state.selectedCategories.size < 3,
                    modifier = Modifier,
                    iconSize = 16.dp,
                    shape = RoundedCornerShape(Theme.radius.full),
                    onClick = {
                        if (state.selectedCategories.contains(category)) {
                            listener.onCategoryDeselected(category)
