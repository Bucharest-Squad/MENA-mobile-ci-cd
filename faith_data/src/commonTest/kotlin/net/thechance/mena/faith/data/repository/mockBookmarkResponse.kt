...
internal val fakeBookmarkList = listOf(
    AyahBookmarkDto(
        id = "1",
        surahId = 1,
        ayahNumber = 1,
...
internal fun MockRequestHandleScope.getBookmarksResponse() = respond(
    content = jsonSerialization.encodeToString(
        PageResponse.serializer(AyahBookmarkDto.serializer()),
        PageResponse(
            currentPage = 0,
            items = fakeBookmarkList,
            totalPages = 1,
            totalItems = fakeBookmarkList.size,
        )
    ),
...
internal fun MockRequestHandleScope.postBookmarkResponse(
    body: AyahBookmarkDto = fakeBookmarkList.first()
) = respond(
    content = jsonSerialization.encodeToString(
